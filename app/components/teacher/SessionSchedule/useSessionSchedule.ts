...
import { ActionButton } from "@/ui/Card/ClassScheduleCard";
  ...
  return useMemo(() => {
    ...
    return data.map((session) => {
      ...
      // 3) 오늘
        ...
        statusLabel = "오늘";
        actions = [
          { label: "날짜 변경", variant: "secondary", handleOnClick: () => {} },
          { label: "휴강 처리", variant: "secondary", handleOnClick: () => {} },
          { label: "과외 완료", variant: "primary", handleOnClick: () => {} },
        ];
      }
      // 4) 미래 수업 일정
      else if (diffDays > 0) {
